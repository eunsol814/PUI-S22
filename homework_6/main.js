...
function Appointment(type, method, time, location, address, id) {
    this.type = type;
    this.method = method;
    this.time = time;
...
// localStorage.clear();
var a = new Appointment("test", "RT-PCR", new Date(2022, 2, 10, 10, 30, 0), "CVS", "5600 Wilkins Ave, Pittsburgh, PA 15217", "1");
a.result = "Negative";
var b = new Appointment("vaccine", "Pfizer - 2nd Dose", new Date(2021, 8, 15, 12, 0, 0), "CVS", "5600 Wilkins Ave, Pittsburgh, PA 15217", "2");
b.result = "Completed";
var c = new Appointment("vaccine", "Pfizer - Booster Shot", new Date(2022, 1, 3, 12, 0, 0), "CVS", "5600 Wilkins Ave, Pittsburgh, PA 15217", "3");
c.status = false;
// var d = new Appointment("vaccine", "Pfizer - Booster Shot", new Date(2022, 3, 3, 14, 30, 0), "CVS", "5600 Wilkins Ave, Pittsburgh, PA 15217", "4");
...
function loadMain() {
    ...
    for (var i=0; i<localStorage.length; i++) {
        sortedAppointments.push(JSON.parse(localStorage.getItem(localStorage.key(i))))
    }
...
function loadCalendar() {
    ...
    var myCalendar = jsCalendar.new({
        ...
        dayFormat : "DD",
        firstDayOfTheWeek : "2",
        min: new Date(new Date(Date.now()).toDateString()),
    });
    myCalendar.onDateRender(function(date, element, info) {
        if (date < new Date(new Date(Date.now()).toDateString())) {
...
window.addEventListener('click', (e) => {
    ...
    if (e.target.id == "confirm") {
        ...
        if (e.target.classList.contains("testing-confirm")) {
            document.querySelector(".testing").classList.add("hidden");
            type = "test";
        } else if (e.target.classList.contains("vaccination-confirm")) {
            document.querySelector(".vaccination").classList.add("hidden");
            type = "vaccine";
        }
        e.target.classList.add("hidden");
        // Add appointment to local storage
        var method = document.querySelector('input[type=radio][name=method]:checked').value;
        var time = selectedDate + " " + slot.querySelector(".slot-time").innerHTML;
        var location = slot.querySelector(".slot-location").innerHTML;
        var id = Date.now().toString();
        var newAppointment = new Appointment(type, method, time, location, "5600 Wilkins Ave, Pittsburgh, PA 15217", id);
        localStorage.setItem(id, JSON.stringify(newAppointment));
    }
})